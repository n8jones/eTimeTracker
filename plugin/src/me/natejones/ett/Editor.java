...
public class Editor extends EditorPart{
	...
	public void createPartControl(Composite parent) {
		...
		keywordTxt.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		
		Label lblDateRange = new Label(grpFilter, SWT.NONE);
		lblDateRange.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		lblDateRange.setText("Date Range:");
		
		final Combo dateRangeType = new Combo(grpFilter, SWT.READ_ONLY);
		dateRangeType.setItems(new String[] {ALL, TODAY, THIS_WEEK, THIS_MONTH, CUSTOM});
		dateRangeType.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false, false, 1, 1));
		dateRangeType.select(0);
		
		final Composite customDateRangeOptions = new Composite(grpFilter, SWT.NONE);
		customDateRangeOptions.setLayoutData(new GridData(SWT.FILL, SWT.FILL, false, false, 1, 1));
		...
		final DateTime startDateTime = new DateTime(customDateRangeOptions, SWT.BORDER | SWT.DROP_DOWN);
		
		Label lblEndTime = new Label(customDateRangeOptions, SWT.NONE);
		lblEndTime.setText("End Time:");
		
		final DateTime endDateTime = new DateTime(customDateRangeOptions, SWT.BORDER | SWT.DROP_DOWN);
		...
		SelectionListener dateFilterChanged = new SelectionListener(){
			...
			public void widgetSelected(SelectionEvent e) {
				Editor.this.updateDateRange(dateRangeType.getText(), startDateTime, endDateTime);
				tableViewer.refresh();
			}
		...
		tableViewer.addFilter(new ViewerFilter(){
			...
			public boolean select(Viewer viewer, Object parentElement,
					Object element) {
				if(keywordTxt.isDisposed()) return true;
				String keyword = keywordTxt.getText();
				if(Strings.isNullOrEmpty(keyword)) return true;
	...
	private void updateDateRange(String rangeType, DateTime start, DateTime end){
		if(Strings.isNullOrEmpty(rangeType)) return;
		...
		if(TODAY.equals(rangeType)){
			...
			if((start == null) || start.isDisposed()) return;
			if((end == null) || end.isDisposed()) return;
